...
openai_api_key = "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"  # Ensure this is securely stored and not hard-coded
...
def correct_text_with_gpt(text, openai_api_key):
    ...
    try:
        response = openai.Completion.create(
            ...
            prompt=f"Please correct the grammar and spelling mistakes in the following text:\n\n{text}",
            max_tokens=150
        )
        corrected_text = response.choices[0].text.strip()
        return corrected_text
    except Exception as e:
        print("Error in GPT text correction:", e)
...
with tab5:
    ...
    st.write("""
    SY-IT(B) 
             Group Members - Rishi Jain, Pranav Patel, Ishita Jasuja, Tanaya Joshi, Kimaya Joshi.
